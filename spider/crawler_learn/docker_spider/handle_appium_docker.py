...
def handle_appium(info):
    ...
    #今日头条
        ...
        if WebDriverWait(driver, 60).until(lambda x: x.find_element_by_xpath("//android.view.View[@content-desc='推荐']")):
            while True:
                # 初始鼠标位置，从哪开始，结束时鼠标位置，到哪结束
                driver.swipe(x1, y1, x1, y2)
                time.sleep(3)
